from utils import Caltech256Dataset, SquarifyImage, RandomCrop, ToTensor, Normalize
...
def test_data_squarify():
    data_set = Caltech256Dataset(root_dir="../caltech256/256_ObjectCategories", transform=SquarifyImage())
...
def test_data_random_crop():
    data_set = Caltech256Dataset(root_dir="../caltech256/256_ObjectCategories",
                                 transform=transforms.Compose([SquarifyImage(), RandomCrop(224)]))
    idx = np.random.randint(0, 2000)
    sample = data_set[idx]
...
def test_data_batch():
    data_set = Caltech256Dataset(root_dir="../caltech256/256_ObjectCategories",
                                 transform=transforms.Compose([SquarifyImage(), RandomCrop(224), ToTensor()]))
